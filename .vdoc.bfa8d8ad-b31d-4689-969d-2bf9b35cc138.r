...
###there are two columns that report p-values, one is "p_value_t" and the other is "p_value_f". count the number of observations in both combined that are reported, meaning that they contain either a numeric value or the letter "y". This will give us an estimate of how many p-values were reported in total, regardless of whether they were reported in the t-test column or the F-test column. We can then break this down by test type to see if there are differences in reporting practices between t-tests and F-tests.
is_reported_value <- function(x) {
  s <- as.character(x)
  s_trim <- str_trim(s)
  is_missing <- is.na(s) | s_trim == ""
...
p_value_summary <- Coded_papers |>
  mutate(reported_p = is_reported_value(p_value_t) | is_reported_value(p_value_f)) |>
  group_by(Test_Type) |>
  summarise(
    reported = sum(reported_p, na.rm = TRUE),
    missing = sum(!reported_p, na.rm = TRUE),
    total = n(),
    proportion_reported = reported / total,
    perc_of_total_reported = reported / total_sample * 100,
    perc_of_total_missing = missing / total_sample * 100,
