...
It returns all p-values (I thought it was only significant)
```{r}
exact <- module_run(
  articles_files, 
  "stat_p_exact"
...
`all_ps` and `exact$table |> count(text, sort = TRUE)` indeed return the same number of rows. This is because they do exactly the same.

To count how many p-values were reported imprecisely, do this:
```{r}
sum(exact$table$imprecise)
```
So there were 1445 p-values reported imprecisely.

To check whether the module is doing what it is supposed to do, we can compare the `text` column with the `imprecise` column. For example, for the first 5 cases, the p-values were reported as follows:
```{r}
exact$table[1:5, "text"]
```
To check whether p-values were classified correctly, do this:
```{r}
exact$table[1:5, "imprecise"]
```
We can see that the first, second and fourth p-values were reported imprecisely so they are evaluated as TRUE.

If you want to see the sentence, do this:
```{r}
head(exact$table$expanded, 5)
```
The 5 p-values reported in sentences 1-3 correspond to the first 5 p-values obtained by running `exact$table[1:5, "text"]`.


To assess the number of imprecisely reported significant and non-significant p-values we need to do some data wrangling.

#### Count imprecise non-sisgnificant p-values
```{r}
exact$table |> 
  filter(p_comp %in% c(">", "≥")) |> 
  count()
...
#### Count imprecise significant p-values
```{r}
exact$table |> 
  filter(p_comp %in% c("<", "≤")) |> 
  count()
